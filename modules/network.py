...
class Network:
    ...
    def Interfaces(self, iface):
        # list all interfaces
        self.iface = iface
        self.process = subprocess.Popen(['ip', 'addr', 'show', 'dev', '%s' % (
            self.iface)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def AddIface(self, iface_ip, netmask, iface_name, db):
        ...
        if self.iface_ip in self.ips or self.iface_ip == '127.0.0.1':
            ...
            # Add interface
            self.process2 = subprocess.Popen(['sudo', '/sbin/ifconfig', str(self.name), str(
                self.iface_ip), 'netmask', str(self.netmask)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            self.out, self.err = self.process2.communicate()
            message = T('Interface Added')
