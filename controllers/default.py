...
def GeneralConfig():
    ...
    if q:
        form = SQLFORM(db.general_config, record,
    ...
    test_smtp = SQLFORM(db.dummy, dbio=False, labels=labels,
                        formstyle='bootstrap4_stacked')
...
def SaveCRSConf():
    ...
    if b != 'YES':
        ...
        if query:
            ...
            record = ((db.rules.id_rand == query.id_rand) &
                      (db.rules.rule_name == 'crs-setup.conf'))
            unquoted_body = urllib.parse.unquote(request.vars.body)
            db.rules.update_or_insert(
                record, rule_name='crs-setup.conf', id_rand=query.id_rand, body=unquoted_body)
            UpdateFiles = stuffs.CreateFiles()
            UpdateFiles.CreateModsecRule(
                crs_path, 'crs-setup.conf', unquoted_body)
    ...
    if b != 'YES':
        ...
        if query:
            ...
            if rule_name in rules:
                record = ((db.rules.id_rand == query.id_rand) & (
                    db.rules.rule_name == request.args(1).upper()))
                unquoted_body = urllib.parse.unquote(request.vars.body)
...
def RuleEdit():
    ...
    if b != 'YES':
        ...
        if query:
            ...
            if rule_name in rules:
                query_rule = db((db.rules.id_rand == request.args(0)) & (
                    db.rules.rule_name == request.args(1).upper())).select(db.rules.body).first()
                #print('query rule')
                # print(query_rule)
                try:
...
def GetActiveConnections():
    ...
    for cc in c:
        data.append([str(cc, 'utf-8').split('->')[0],
                     str(cc, 'utf-8').split('->')[1]])
...
def ManageAllRules():
    ...
    if b == 'NO':
        logger.Logger.NewLogError(
            db2, auth.user.username, "Manage all rules: Invalid characters in data received - Debug: %s" % (b))

    if b == 'YES':
        rules_enabled = 'ls /opt/waf/nginx/etc/modsec_rules/' + \
            query[0]['app_name'] + \
            '/enabled_rules | grep  ".conf$" | sed "s/.conf//g"'
        r = subprocess.Popen(rules_enabled, shell=True,
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        rules_that_exist, rules_that_exist_err = r.communicate()
        rules_that_exist = rules_that_exist.replace(
...
def ManageRules():
    ...
    try:
        ...
        logger.Logger.NewLogError(
            db2, auth.user.username, "Manage rules: Invalid characters in data received - Debug: %s %s %s" % (b, c, d))
    ...
    if b == 'YES' and c == 'YES' and request.vars['action'] in ['On', 'Off']:
        ...
        if query:
            rulest_that_exist = 'cat /opt/waf/nginx/etc/modsec_rules/' + \
            ...
            for x in f:
                ...
                if request.vars['rule'] == x:
                    q = db((db.rules.id_rand == request.vars['id']) & (
                        db.rules.rule_name == request.vars['rule'])).select()
                    rule = request.vars['rule'] + ".conf"
                    if request.vars['action'] == 'On':

                        os.system('cp /opt/waf/nginx/etc/modsec_rules/%s/rules/%s /opt/waf/nginx/etc/modsec_rules/%s/enabled_rules/%s' %
                                  (query[0]['app_name'], rule, query[0]['app_name'], rule))
                        reload_waf.Reload()

                    else:
                        os.system(
                            'rm /opt/waf/nginx/etc/modsec_rules/%s/enabled_rules/%s' % (query[0]['app_name'], rule))
                        reload_waf.Reload()
                    if q:
                        db((db.rules.id_rand == request.vars['id']) & (
                            db.rules.rule_name == request.vars['rule'])).update(status=request.vars['action'])
                    else:
                    ...
                    logger.Logger.NewLogApp(
                        db2, auth.user.username, "Manage rules: Rule: "+request.vars['rule'] + " " + request.vars['action'])
                    msg = 'Done'
                    break

    else:
        msg = "Manage rules: Invalid characters in data received - Debug: %s %s %s" % (
            b, c, d)
        logger.Logger.NewLogError(
            db2, auth.user.username, "Manage rules: Invalid characters in data received - Debug: %s %s %s" % (b, c, d))
...
def Dashboard():
    ...
    for r in row:
        stats.append({'app_name': r.app_name, 'id_rand': r.id_rand, 'critical': r.critical,
                      'warning': r.warning, 'notice': r.notice, 'error': r.error, 'total_requests': r.total_requests})

    return dict(page=T("Dashboard"), icon="fa fa-bar-chart", title="", swap_percent=swap_percent, swap_free=swap_free, swap_total=swap_total, swap_used=swap_used, ram_percent=ram_percent, ram_free=ram_free, ram_total=ram_total, ram_used=ram_used, cpu_percent=cpu_percent, disk_total=disk_total, disk_used=disk_used, disk_free=disk_free, disk_percent=disk_percent, summary=stats,)
...
def ProdEdit():
    ...
    if b == 'YES':
        ...
        if query[0]['app_name']:
            ...
            rules_base = 'ls /opt/waf/nginx/etc/modsec_rules/' + \
                query[0]['app_name'] + \
                '/rules | grep  ".conf$" | sed "s/.conf//g"'
            r = subprocess.Popen(rules_base, shell=True,
                                 stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            rules_that_exist, rules_that_exist_err = r.communicate()
...
def Websites():
    links = ((
        ...
        (lambda row: TAG.a(' ', _href=URL('default', 'ProdEdit/' + str(row.id_rand)),
                           target="callback-command", _class="btn btn-info glyphicon glyphicon-pencil")),
        (lambda row: TAG.a(' ', _href=URL('default', 'EnableApp/' + str(row.id_rand)),
                           target="callback-command", _class="btn btn-success glyphicon glyphicon-play")),
        (lambda row: TAG.a(' ', _href=URL('default', 'DisableApp/' + str(row.id_rand)),
                           target="callback-command", _class="btn btn-warning glyphicon glyphicon-stop")),
        (lambda row: TAG.a(' ', _href=URL('default', 'DeleteApp/' + str(row.id_rand)),
                           target="callback-command", _class="btn btn-danger glyphicon glyphicon-trash")),
    )
    headers = {'production.app_name': 'Name'}
    fields = [db.production.app_name, db.production.listen_ip,
              db.production.autor, db.production.id_rand, db.production.enabled]
    # Disable some camps in grid
    db.production.id_rand.writable = False
    db.production.id_rand.readable = False
    db.production.id_rand.writable = False
    db.production.id_rand.readable = False
    grid = SQLFORM.grid(db.production, links=links, headers=headers, details=False, fields=fields,
                        searchable=False, csv=False, create=False, editable=False, deletable=False)
...
def DeleteApp():
    ...
    if b == 'YES':
        ...
        query = db(db.production.id_rand == request.args[0]).select(
            db.production.app_name, db.production.vhost_id, db.production.listen_ip)
        ...
        subprocess.Popen(['sudo', 'chown', 'www-data.www-data', '/home/www-data/waf2py_community/applications/Waf2Py/logrotation.d/%s.conf' %
                          (query[0]['app_name'])], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        is_used = db(db.system.iface_ip == query[0]['listen_ip']).select(
            db.system.used_by)
        if is_used:
            if is_used[0]['used_by'] == query[0]['app_name']:
                db(db.system.iface_ip == query[0]['listen_ip']).update(
                    available='Available', used_by=None)
            else:
                is_used_update = is_used[0]['used_by'].replace(
                    ", " + query[0]['app_name'], "")
                is_used_update = is_used_update.replace(
                    query[0]['app_name'] + ", ", "")
                is_used_update = is_used_update.replace(
                    query[0]['app_name'], "")
                db(db.system.iface_ip == query[0]['listen_ip']).update(
                    used_by=is_used_update)
        ...
        if r == "Reload Succesfull":
            ...
            logger.Logger.NewLogApp(
                db2, auth.user.username, 'Application was' + query[0]['app_name'] + 'deleted')
            logger.Logger.NewLogApp(db2, auth.user.username, r)
        else:
            resp = r
        logger.Logger.NewLogApp(db2, auth.user.username,
                                "Delete app " + query[0]['app_name'])
        response.flash = resp
        session.disabled = ''
        session.d_expanded = 'false'
        session.enabled = 'active'
        ...
        if request.vars['app_url'] != '' and len(request.vars['app_url']) < 45 and request.vars['name'] != '' and len(request.vars['name']) < 45:
            ...
            # Check if name contains dangerous character
                ...
                if query:
                    ...
                    # Deploy
                    app_id = db.new_app.insert(name=request.vars.name, app_name=AppName, id_rand=id_rand, max_fails='1',
                                               fail_timeout='60', autor=session['auth']['user']['username'], checked=1, deployed=0)
...
def deploy():
    ...
    if b == 'YES':
        ...
        nginx_conf_data = query.nginx_conf_data.replace(
            'vhost_id', str(app_id)).replace('plbsid_id', str(app_id))
        #modsec_conf_data = query.modsec_conf_data.replace('vhost_id', str(app_id))
        db(db.production.id == app_id).update(vhost_id=app_id,
                                              plbsid_id=app_id, nginx_conf_data=nginx_conf_data)

        # Create Nginx default conf for new App
        CreateNginx = stuffs.CreateFiles()

        response = CreateNginx.CreateNginxFiles(
            ProdNginxAvail, AppName, nginx_conf_data)

        # Create folders and rules for new app
        CreateModsec = stuffs.CreateFiles()
        response = CreateModsec.CreateModsecFiles(
            ProdModsecRules, AppName, ProdModsecConf, query.modsec_conf_data)
        ...
        rules_enabled = 'ls /opt/waf/nginx/etc/modsec_rules/' + \
            AppName+'/enabled_rules | grep  ".conf$" | sed "s/.conf//g"'
        r = subprocess.Popen(rules_enabled, shell=True,
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        rules_that_exist, rules_that_exist_err = r.communicate()
        ...
        # create logs folder
        subprocess.Popen(
            ['mkdir', '-p', '/opt/waf/nginx/var/log/%s/' % (AppName)])

        # create deny paths and headers directory
        subprocess.Popen(
            ['mkdir', '-p', '/opt/waf/nginx/etc/rewrite/paths/%s' % (AppName)])
        ...
        db.commit()
        logger.Logger.NewLogApp(
            db2, auth.user.username, "Deploye Application: " + query.app_name + 'deployed')
        redirect(URL('Websites'))
...
def DeleteNewApp():
    ...
    if b == 'YES':
        ...
        # Get App Name
        query = db(db.new_app.id_rand == request.args[0]).select(
            db.new_app.app_name)
        if query:
            ...
            db(db.new_app.id_rand == request.args[0]).delete()
            logger.Logger.NewLogApp(
                db2, auth.user.username, "Deleted App: Deleted " + AppName)
...
@auth.requires_login()
def new_app():
    query = db(db.new_app).select(db.new_app.app_name, db.new_app.autor,
                                  db.new_app.name, db.new_app.id_rand, db.new_app.checked)

    return dict(query=query, page=T("Add a new application"), icon="fa fa-plus", title=T("Create a new application"))


@auth.requires_login()
def status():
    listening = subprocess.Popen(
        ['sudo', 'netstat', '-tulpen'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out1, err1 = listening.communicate()
    p1 = subprocess.Popen(
        ['ps', 'aux'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    p2 = subprocess.Popen(
        ['grep', 'nginx'], stdin=p1.stdout, stdout=subprocess.PIPE)
...
def EngxEdit():
    ...
    if b == 'YES':
        query = db(db.production.id_rand == request.args[0]).select(
            db.production.nginx_conf_data, db.production.app_name).first()
        if query:
            db(db.production.id_rand == request.args[0]).update(
                nginx_conf_data=request.vars.body)
            DataNginx = query.nginx_conf_data
            AppName = query.app_name
            UpdateFiles = stuffs.CreateFiles()
            UpdateFiles.CreateNginxFiles(
                ProdNginxAvail, AppName, request.vars.body)
            #UpdateFiles.CreateNginxSymlink(ProdNginxAvail, AppName, 'prod')
            response.flash = 'Configuration Saved'
            r = stuffs.Nginx()
            r.Reload()
            logger.Logger.NewLogApp(
                db2, auth.user.username, "EngxEdit: prod saved configuration app: " + AppName)
        else:
            response.flash = 'App does not exist'
...
def ModsEdit():
    ...
    if b == 'YES':
        ...
        query = db(db.production.id_rand == request.args[0]).select(
            db.production.modsec_conf_data, db.production.app_name).first()
        if query:
            db(db.production.id_rand == request.args[0]).update(
                modsec_conf_data=request.vars.body)
...
def BackendIps():
    ...
    if b == 'YES' and request.vars['http'] or request.vars['https']:
        ...
        os.system('echo "" > /opt/waf/nginx/etc/backend/%s.conf ' %
                  (query[0]['app_name']))
        http_backend = urllib.parse.unquote(request.vars['http']).splitlines()
        https_backend = urllib.parse.unquote(
            request.vars['https']).splitlines()
        ...
        for b in https_backend:
            ...
            if "=>" not in b:
                ...
                https_backend_hosts += '\n'
                https_backend_hosts += str((backend_ip[0])) + \
                    ':'+str((backend_ip[1]))
        ...
        if not http_backend:
            db(db.production.id_rand == request.vars['id']).update(
                backend_ip_http="{}".format(''))

        if not https_backend:
            db(db.production.id_rand == request.vars['id']).update(
                backend_ip_https="{}".format(''))

        if HttpBckend:
            db(db.production.id_rand == request.vars['id']).update(
                backend_ip_http="{}".format(http_backend_hosts.replace(':', '=>')))
            r = create_backend.CreateBackend(str(query[0]['app_name']), "{}".format(http_backend_hosts),
                                             str(query[0]['vhost_id']
                                                 ), query[0]['max_fails'],
                                             query[0]['fail_timeout'], query[0]['vhost_id'], 'http')

        if HttpsBckend:
            db(db.production.id_rand == request.vars['id']).update(
                backend_ip_https="{}".format(https_backend_hosts.replace(':', '=>')))
...
def EnableApp():
    ...
    if d == 'YES':
        query = db(db.production.id_rand == request.args[0]).select(
            db.production.app_name, db.production.listen_ip)
        AppName = str(query[0]['app_name'])
...
def Listen():
    ...
    if '-' in request.vars['http_ports']:
        ...
        d = 'YES' if IS_INT_IN_RANGE(1, 65535)(
            request.vars['http_ports'])[1] == None else 'NO'
    ...
    if '-' in request.vars['https_ports']:
        ...
        e = 'YES' if IS_INT_IN_RANGE(1, 65535, error_message='Invalid port')(
            request.vars['https_ports'])[1] == None else 'NO'
    ...
    if b == 'YES' and c == 'YES' and (d == 'YES' or e == 'YES') == True:
        ...
        # Get actual ip
        actual_ip = db(db.production.id_rand == request.vars['id']).select(
            db.production.listen_ip)
        ...
        try:
            ...
            if request.vars['listen_ip'] in ips_list:
                ...
                os.system('rm %s%s/listenHTTP.conf' %
                          (ListenPATH, query[0]['app_name']))
                os.system('rm %s%s/listenHTTPS.conf' %
                          (ListenPATH, query[0]['app_name']))

                # if ip is different
                if actual_ip[0]['listen_ip'] != request.vars['listen_ip']:
                    # check if other app is using this ip and get the app_name
                    is_used_by = db(db.system.iface_ip == actual_ip[0]['listen_ip']).select(
                        db.system.used_by)
                    if is_used_by:
                        is_used_by = is_used_by[0]['used_by'].replace(
                            ", "+query[0]['app_name'], "")
                        is_used_by = is_used_by.replace(
                            query[0]['app_name']+", ", "")
                        is_used_by = is_used_by.replace(
                            query[0]['app_name'], "")
                        ...
                        if is_used_by == "":
                            is_used_by = None
                            db(db.system.iface_ip == actual_ip[0]['listen_ip']).update(
                                used_by=is_used_by, available='Available')
                        else:
                            db(db.system.iface_ip == actual_ip[0]['listen_ip']).update(
                                used_by=is_used_by)
                    else:
                    ...
                    new_ip_used_by = db(db.system.iface_ip == request.vars['listen_ip']).select(
                        db.system.used_by)
                    # print "new_ip_used_by: ", new_ip_used_by[0]
                    if new_ip_used_by[0]['used_by'] != None:
                        new_ip_used_by = new_ip_used_by[0]['used_by'] + \
                            ", "+query[0]['app_name']
                        # update list of apps using this ip
                        db(db.system.iface_ip == request.vars['listen_ip']).update(
                            used_by=new_ip_used_by, available='In use')

                    else:
                        # print 'Else'
                        db(db.system.iface_ip == request.vars['listen_ip']).update(
                            used_by=query[0]['app_name'], available='In use')

                    # Update production ip with the new ip
                    db(db.production.id_rand == request.vars['id']).update(
                        listen_ip=request.vars['listen_ip'])
                    IpChanged = True
                ...
                if d == 'YES' and e == 'NO':
                    ...
                    if multi_http_ports:
                        db(db.production.id_rand == request.vars['id']).update(
                            ports_http=request.vars['http_ports'].replace("\n", "-"))
                    db(db.production.id_rand == request.vars['id']).update(
                        ports_https='')
                    f = open('%s%s/listenHTTP.conf' %
                             (ListenPATH, query[0]['app_name']), 'a')
                    for port in request.vars['http_ports'].splitlines():
                        # print "Adding configuration"
                        f.write('listen %s:%s;' %
                                (request.vars['listen_ip'], port))
                        f.write('\n')
                    f.close()
                    # Comment SSL certitificate line
                    if "#ssl_certificate" not in query[0]['nginx_conf_data']:
                        query[0]['nginx_conf_data'] = query[0]['nginx_conf_data'].replace(
                            ' ssl_certificate', '#ssl_certificate')

                    UpdateFiles.CreateNginxFiles(
                        ProdNginxAvail, query[0]['app_name'], query[0]['nginx_conf_data'])

                    db(db.production.id_rand == request.vars['id']).update(
                        nginx_conf_data=query[0]['nginx_conf_data'])
                    logger.Logger.NewLogApp(db2, auth.user.username, "Listening: {} -> {} - Ports: {}".format(
                        query[0]['app_name'],  request.vars['listen_ip'], request.vars['http_ports']))
                    if IpChanged:
                ...
                # If https ports are selected and valid, it will be created the listen file config
                    ...
                    # update https ports
                    db(db.production.id_rand == request.vars['id']).update(
                        ports_https=request.vars['https_ports'].replace("\n", "-"))
                    db(db.production.id_rand ==
                       request.vars['id']).update(ports_http='')
                    f = open('%s%s/listenHTTPS.conf' %
                             (ListenPATH, query[0]['app_name']), 'a')
                    for port in request.vars['https_ports'].splitlines():
                        f.write('listen %s:%s ssl;' %
                                (request.vars['listen_ip'], port))
                        f.write('\n')
                    f.close()
                    # Create dir ssl if not exist
                    Path(SslPATH + query[0]['app_name']
                    ...
                    db(db.production.id_rand == request.vars['id']).update(
                        nginx_conf_data=query[0]['nginx_conf_data'])
                    logger.Logger.NewLogApp(db2, auth.user.username, "Listening: {} -> {} - Ports: {}".format(
                        query[0]['app_name'],  request.vars['listen_ip'], request.vars['https_ports']))
                    if IpChanged:
            ...
            logger.Logger.NewLogError(db2, auth.user.username, "ERROR in Listen function for app {}: {}".format(
                query[0]['app_name'], str(e)))
...
def Mode():
    ...
    if id_rand == 'YES' and alias_mode in modes:
        ...
        modsec = db(db.production.id_rand == request.vars['id']).select(
        ...
        r = change.Text(modsec_data, 'SecRuleEngine',
                        "SecRuleEngine %s" % (mode))
        db(db.production.id_rand == request.vars['id']).update(
            modsec_conf_data='\n'.join(r['new_list']), mode=alias_mode)

        # get new conf
        new = db(db.production.id_rand == request.vars['id']).select(
            db.production.modsec_conf_data)

        UpdateFiles = stuffs.CreateFiles()
        try:
            UpdateFiles.CreateModsecConf(
                'prod', modsec[0]['app_name'], new[0]['modsec_conf_data'])
            logger.Logger.NewLogApp(
                db2, auth.user.username, "Mode: prod " + modsec[0]['app_name'])
        except Exception as e:
            logger.Logger.NewLogError(
                db2, auth.user.username, "Mode: " + str(e))
            session.flash = e
