...
for app in b:
    d = con.execute(
        'SELECT size FROM log_size WHERE id_rand="{}" AND log_type="modsec"'.format(app[1]))
    old_size = d.fetchall()
    actual_size = os.path.getsize(
        '/opt/waf/nginx/var/log/{}/{}_audit.log'.format(app[0], app[0]))
    #print('old:{} new:{}'.format(str(old_size[0]), str(actual_size)))
    if old_size[0][0] < actual_size:
        #print('bigger file')
        f = open('/opt/waf/nginx/var/log/{}/{}_audit.log'.format(
            app[0], app[0]), 'r', encoding='utf8', errors='ignore')
        x = f.read()
        f.close()
        notice = 0
        warning = 0
        ...
        for i in transactions:
            ...
            for ii in row['transaction']['messages']:
                if ii['details']['severity'] == '0':
                    ...
                    critical += 1
        con.execute('UPDATE summary SET critical={}, warning={}, notice={}, error={}, total_requests={} WHERE id_rand="{}"'.format(
            int(critical), int(warning), int(notice), int(error), int(total), app[1]))
        con.execute('UPDATE log_size SET size={} WHERE id_rand="{}" AND log_type="modsec"'.format(
            int(actual_size), app[1]))
